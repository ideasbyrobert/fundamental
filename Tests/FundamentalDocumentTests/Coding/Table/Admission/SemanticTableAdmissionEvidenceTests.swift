...
extension SemanticTableAdmissionTests
{
    ...
    {
        let missing = try Self.sourced(Self.table())
        let blank = try Self.sourced(Self.table(
            sourceLocation: " \t\n "
        ))
        let exactValue = "  table:2  "
        let exact = try Self.sourced(Self.table(
        ...
        #expect(!missing.evidence.facts.contains(.repair(blankRepair)))
        #expect(blank.evidence.facts.contains(.repair(blankRepair)))
        #expect(exact.evidence.facts.contains(.sourceLocation(
            target: .table,
            location: location
        )))
    }
    ...
    func validConfidenceAndNestedEvidenceRetainExactTargets() throws
    {
        for value in [0.0, 0.25, 1.0]
        {
            ...
            #expect(admission.evidence.facts.contains(.confidence(
                target: .table,
                confidence: tableConfidence
            )))
            #expect(admission.evidence.facts.contains(.sourceLocation(
