...
@Suite("An applied semantic text replacement")
struct AppliedSemanticTextReplacementTests
{
    @Test("forward and reverse ranges replace the same scalar interval")
    func rangeDirectionDoesNotChangeReplacement() throws
    {
        for bounds in [(1, 3), (3, 1)]
        {
            ...
            #expect(try Self.text(in: result) == "AXD")
    ...
    func sourceDocumentRemainsUnchanged() throws
    {
        ...
        #expect(try Self.text(in: result) == "Newginal")
    }
}
