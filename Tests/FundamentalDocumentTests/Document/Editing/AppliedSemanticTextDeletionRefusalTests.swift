...
extension AppliedSemanticTextDeletionTests
{
    ...
    func invalidInputsRefuseAtomically() throws
    {
        ...
        let original = source
        let requests = try [
            Self.deletion(start: 0, end: 1, documentMarker: 9),
            Self.deletion(start: 0, end: 1, revision: 9),
            Self.deletion(start: 0, end: 1, blockMarker: 9),
            Self.deletion(start: 0, end: 5),
            Self.deletion(start: 3, end: 4)
        ]
        ...
        let mixedRequest = try Self.deletion(start: 0, end: 1)
        ...
        let tableRequest = try Self.deletion(start: 0, end: 1)
        #expect(AppliedSemanticTextEdit(
            tableRequest,
            in: tableDocument
        ) == nil)
