...
{
    @Test("custom input survives native scroll resize and appearance")
    func nativeLifecycle() throws
    {
        let source = try MacReaderDocumentFixture.source((0 ..< 300).map
        {
            MacReaderDocumentFixture.paragraph(
                "Paragraph \($0): " +
                String(repeating: "Exact Unicode e\u{301} 😀 Раздел. ", count: 5)
            )
        })
        let retained = source
        ...
        try MacReaderDocumentFixture.capture(controller, name: "scrolled-dark")
    }
}
