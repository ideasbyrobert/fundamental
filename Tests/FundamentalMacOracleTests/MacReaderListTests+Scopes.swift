...
{
    @Test("native list copying preserves all four Unicode run scopes",
          arguments: SemanticListKind.allCases)
    func scopedSource(kind: SemanticListKind) throws
    {
        let source = try MacReaderDocumentFixture.source([
            .listItem(SemanticListItem(
                kind: kind, runs: MacReaderDocumentFixture.runs(
        ...
        try MacReaderDocumentFixture.capture(
            controller, name: "list-scoped-copy-\(kind)"
        )
    }
}
