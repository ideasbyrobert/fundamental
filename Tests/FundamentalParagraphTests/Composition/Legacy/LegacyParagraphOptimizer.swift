...
@MainActor
struct LegacyParagraphOptimizer
{
    let cache: ParagraphEdgeCache
    let width: Double

    func optimize() throws -> ParagraphPath
    {
        ...
        let empty = [ParagraphReachability](
            repeating: .unavailable, count: ParagraphFitness.allCases.count
        )
        var states = [[ParagraphReachability]](
        ...
        for end in 1..<points.count
        {
            for start in 0..<end
            {
                ...
                for fitness in ParagraphFitness.allCases
                {
                    ...
                    transitions += 1
                    let score = try previous.score.appending(
    ...
    private static func reached(_ state: ParagraphReachability) -> Bool
    {
