package struct PresentationPixelBounds: Equatable, Sendable
{
    ...
    )
    {
        ...
        let (width, widthOverflow) = maximumX
            .subtractingReportingOverflow(minimumX)
        let (height, heightOverflow) = maximumY
            .subtractingReportingOverflow(minimumY)
        let (area, areaOverflow) = width
